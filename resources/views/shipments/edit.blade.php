...
<div class="bg-white shadow-sm border-b">
    <div class="container py-4">
        <h1 class="h2 fw-bold text-dark mb-1">Edit Pengiriman</h1>
        <p class="text-muted mb-0">Perbarui informasi pengiriman {{ $shipment->tracking_number }}</p>
    </div>
</div>
                ...
                <div class="card-body p-4">
                    <form action="{{ route('shipments.update', $shipment) }}" method="POST">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                                        @foreach(\App\Models\Shipment::getStatusOptions() as $value => $label)
                                            <option value="{{ $value }}" 
                                                {{ old('status', $shipment->status) == $value ? 'selected' : '' }}>
                                                {{ $label }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="row g-4">
                                ...
                                <div class="col-12">
                                    ...
                                    <textarea class="form-control @error('sender_address') is-invalid @enderror" 
                                              id="sender_address" name="sender_address" rows="3" required>{{ old('sender_address', $shipment->sender_address) }}</textarea>
                            ...
                            <div class="row g-4">
                                ...
                                <div class="col-12">
                                    ...
                                    <textarea class="form-control @error('recipient_address') is-invalid @enderror" 
                                              id="recipient_address" name="recipient_address" rows="3" required>{{ old('recipient_address', $shipment->recipient_address) }}</textarea>
                        ...
                        <div class="d-flex justify-content-end gap-3 pt-4 border-top">
                            <a href="{{ route('shipments.show', $shipment) }}" class="btn btn-outline-secondary btn-lg px-4">
